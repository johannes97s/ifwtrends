...
keyword = c(NA)
category = c(956, 816,276, 179, 217, 18, 531) #,220,802,1162, 340)



res <- g_index(keyword = keyword, category = category, time = str_c("2006-01-01 ", Sys.Date()))
...
x2 <- as.matrix(res[-1])
#dat[nrow(x2)-1,] <- list(max(dat$time) + months(3), NA)
dat[nrow(x2),] <- list(max(dat$time) + months(3), NA)


index<- bind_cols(dat,as_tibble(predict(model, x2, cv$lambda.min)))%>%
  rename(index = s1)



openxlsx::write.xlsx(index, "Einzelhandel und Index Niveau.xlsx")
